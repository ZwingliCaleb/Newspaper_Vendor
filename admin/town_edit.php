...
$id = $_GET["tid"];
$pid= $_GET["pid"];
$statement = $pdo->prepare("SELECT * FROM town WHERE Town_id=:id");
...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    $name = $_POST["name"];
    $desc = $_POST["desc"];
    $statement = $pdo->prepare("UPDATE town SET Town_name=:name,Town_desc=:desc WHERE Town_id=:id");
    $statement->bindValue(":name",$name);
    $statement->bindValue(":desc",$desc);
    $statement->bindValue(":id",$id);
    $statement->execute();
    header("Location: town.php?mess=updated&id=$pid");
                            ...
                            <div class="card-body p-2">
                                ...
                                <form enctype= multipart/form-data name="myform" action= "" class="row g-3"  method="POST">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <input name="name" type="text" class="form-control" value="<?php echo $country["Town_name"]?>" required>
                                    </div>
                                    <div class="col-md-12">
                                        <label class="form-label">Description</label>
                                        <textarea rows="6" name="desc" style="resize:none; " type="text" class="form-control" id="" required><?php echo $country["Town_desc"]?></textarea>
                                    </div>
